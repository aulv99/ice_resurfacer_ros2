...
def append_segment(path_x, path_y, seg_x, seg_y):
...
def generate_wall_to_sweep_transition(x_wall, y_sweep_top, x_sweep_start):
    """
    Creates a perfect 90-degree right turn to connect Lap 1 to the Sweeps.
    """
    x = []
    y = []
    ...
    append_segment(x, y, sx, sy)
    
    return x, y, y_wall_end
...
def generate_classic_zamboni_sweeps(x_limit, y_top_start, y_bottom_start, lane_spacing, num_passes):
    ...
    for i in range(num_passes):
        ...
        # 2. Right Semicircle at East end (Top to Bottom)
        ...
        # 4. Right Semicircle at West end (Bottom to NEXT Top)
...
def generate_zamboni_path():
    ...
    lx0, ly0 = generate_wall_lap(offset_0, start_y=-6.5, end_y=-5.0)
    px.extend(lx0)
    py.extend(ly0)
    ...
    tx2, ty2, lap1_end_y = generate_wall_to_sweep_transition(
        x_wall=x_wall_lap_1, 
    ...
    # ==========================================
    lx1, ly1 = generate_wall_lap(offset_1, start_y=1.0, end_y=lap1_end_y)
    px.extend(lx1)
    py.extend(ly1)
    ...
    # 6. THE INNER SWEEPS
    # ==========================================
    sweep_x, sweep_y = generate_classic_zamboni_sweeps(
...
class ZamboniPathClient(Node):
    ...
    def send_path(self):
        self.get_logger().info('Generating master pure-geometry trajectory...')
        smooth_x, smooth_y = generate_zamboni_path()

        path_msg = Path()
        path_msg.header.frame_id = 'map'
        path_msg.header.stamp = self.get_clock().now().to_msg()

        self.get_logger().info(f'Converting {len(smooth_x)} coordinates to ROS 2 Poses...')
